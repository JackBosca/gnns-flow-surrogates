...
def render_file(pkl_path, out_dir, fps=None, size=(1920, 1080), skip=1,
    ...
    # crds is (coords, triangles) and build triangulation
    coords, triangles = crds 
    coords = np.asarray(coords)               # (N, >=2)
    tri_arr = np.asarray(triangles)           # (F,3) or (3,F)

    # ensure triangles are (F,3)
    ...
    # default fps to match video duration and elapsed time
    if fps is None:
        fps = 1.0 / (skip * dt)
    ...
    # choose sizes for fonts, scaled by dpi
    title_fs = 12 * (dpi / 400.0)        # title font size
    cbar_label_fs = 10 * (dpi / 400.0)    # colorbar label font size
    cbar_tick_fs = 10 * (dpi / 400.0)     # colorbar tick label size
    axis_label_fs = 10 * (dpi / 400.0)   # x/y axis label font size
    tick_label_fs = 10 * (dpi / 400.0)   # tick numbers font size
    ...
    for step in pbar:
        ...
        fig.subplots_adjust(left=0.1, right=0.9)  # center the layout

        for ax in axes:
            ax.cla()
            ax.set_aspect('equal', adjustable='box')

        # compute per-triangle (face) values by averaging per-node values
        tri_nodes = triang.triangles   # shape (ntri, 3)
        targ_tri = targ_v[tri_nodes].mean(axis=1)
        pred_tri = pred_v[tri_nodes].mean(axis=1)
        diff_tri = diff_v[tri_nodes].mean(axis=1)

        # fill triangles with one color each (flat shading)
        ...
        # draw thin black edges so cells are visible
        for ax in axes:
            ax.triplot(triang, '-', color='k', lw=max(0.2 * (dpi / 100.0), 0.1), ms=0)

        # titles
        axes[0].set_title(fr'$\| v_{{target}} \|$ @ {step * dt:.2f} s', fontsize=title_fs)
        axes[1].set_title(fr'$\| v_{{pred}} \|$ @ {step * dt:.2f} s', fontsize=title_fs)
        axes[2].set_title(fr'$\| v_{{target}} - v_{{pred}} \|$ @ {step * dt:.2f} s', fontsize=title_fs)

        # set axis labels (x/y) for each axes and tick sizes
        for ax in axes:
            ax.set_ylabel(r'$y \; [m]$', fontsize=axis_label_fs, labelpad=4)
        ...
        # set unique x labels only on bottom plot
        axes[2].set_xlabel(r'$x \; [m]$', fontsize=axis_label_fs, labelpad=4)

        # colorbars (shared top two, and one for diff) with labels + tick sizes
        cbar1 = fig.colorbar(h1, ax=[axes[0], axes[1]], orientation='vertical', shrink=0.8)
        cbar1.set_label('Velocity Magnitude [m/s]', fontsize=cbar_label_fs, labelpad=6)
        cbar1.ax.tick_params(labelsize=cbar_tick_fs)

        cbar2 = fig.colorbar(h3, ax=axes[2], orientation='vertical', shrink=1.0)
        cbar2.set_label('Velocity Difference Magnitude [m/s]', fontsize=cbar_label_fs, labelpad=6)
