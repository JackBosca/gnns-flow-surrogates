...
class EGNNStateModel(nn.Module):
    ...
    - final readout head maps per-node features (excluding coordinates) to 5 outputs:
    ...
    def _concat_globals_to_nodes(self, x_nodes, u, batch):
        ...
        Args:
            x_nodes: (N, C_in) node features
            u: (num_graphs, G) or (1, G) global features
            batch: (N,) long tensor mapping nodes -> graph. batch[i] = graph index of node i.
                        Values in [0, num_graphs-1]
        Returns:
            (N, C_in + G) node features with global features concatenated
        """
        if u is None:
            return x_nodes
        ...
        if u.shape[0] == batch.max().item() + 1:
            # each node receives the global feature of its graph
            u_nodes = u[batch]
        elif u.shape[0] == 1:
            # simply repeat for all nodes
    ...
    def forward(self, data):
        ...
        pos = data.pos              # (N, pos_dim) (in 2D case it is (N, 2))
        ...
        x_input = torch.cat([pos, node_feats], dim=-1)  # shape (N, pos_dim + node_feat_dim) = (N, pos_dim + C_in + G)
        ...
        if self.use_separate_heads:
            density  = self.head_density(feats_out).squeeze(-1)  # (N,)
            energy   = self.head_energy(feats_out).squeeze(-1)  # (N,)
            pressure = self.head_pressure(feats_out).squeeze(-1)  # (N,)
